// EducaMadrid
function enableHTMLsignature(){
    var emSignature = $("textarea#rcmfd_signature");
    if (emSignature.length==1 && emSignature.val()=="") {
        $(".editor-toolbar a.mce-i-html").trigger("click");  
    }
}
// / EducaMadrid
/**
...
draft_autosave:0,comm_path:"./",recipients_separator:",",recipients_delimiter:", ",popup_width:1150,popup_width_small:900,thread_padding:"15px"};this.ref="rcmail";var h=this;$.ajaxSetup({cache:!1,timeout:1E3*this.env.request_timeout,error:function(a,b,d){h.http_error(a,b,d)},
// EducaMadrid
// beforeSend:function(a){a.setRequestHeader("X-Roundcube-Request",h.env.request_token)}
beforeSend:function(a){
    var r = window.location.href;
    if (r!='https://'+document.domain+'/' && r.indexOf("/?_task=login")==-1 && r.indexOf("/?_task=logout")==-1) {
        a.setRequestHeader("X-Roundcube-Request",h.env.request_token);
    }
}
// / EducaMadrid
});$(window).on("beforeunload",function(){h.unload=!0});this.set_env=function(a,b){if(null==a||"object"!==typeof a||b)this.env[a]=b;else for(var d in a)this.env[d]=
